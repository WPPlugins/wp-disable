...
class WpPerformance_Admin {
	...
	public function menu() {
		add_management_page( __( 'WP Disable', 'wpperformance' ), __( 'Optimisation.io - Disabler', 'wpperformance' ), 'manage_options', 'wpperformance', array( $this, 'addsettings' ) );
		add_submenu_page( '', __( 'Update Settings', 'wpperformance' ), __( 'Update Settings', 'wpperformance' ), 'manage_options', 'updatewpperformance-settings', array( $this, 'updatesettings' ) );
	}
	...
	public function updatesettings() {
		...
		if ( ! isset( $post_req['wpperformance_admin_settings_nonce'] ) || ! wp_verify_nonce( $post_req['wpperformance_admin_settings_nonce'], 'wpperformance-admin-nonce' ) ) {
			$this->redirect_url( admin_url( 'tools.php?page=wpperformance' ) );
			exit;
		}
		...
		if ( isset( $post_req['disable_gravatars'] ) && 1 === $post_req['disable_gravatars'] ) {
			update_option( 'show_avatars', false );
		} else {
			update_option( 'show_avatars', true );
		}

		if ( isset( $post_req['default_ping_status'] ) && 1 === $post_req['default_ping_status'] ) {
			update_option( 'default_ping_status', 'close' );
		} else {
			update_option( 'default_ping_status', 'open' );
		}

		if ( isset( $post_req['close_comments'] ) && 1 === $post_req['close_comments'] ) {
		...
		if ( isset( $post_req['paginate_comments'] ) && 1 === $post_req['paginate_comments'] ) {
	...
	public function wp_performance_disable_emojis() {
		...
		if ( isset( $settings['disable_emoji'] ) && 1 === $settings['disable_emoji'] ) {
	...
	public function wp_performance_speed_stop_loading_wp_embed() {
		$settings = get_option( WpPerformance::OPTION_KEY . '_settings', array() );
		if ( isset( $settings['disable_embeds'] ) && 1 === $settings['disable_embeds'] ) {
			if ( ! is_admin() ) {
	...
	public function wp_performance_remove_script_version( $src ) {
		$settings = get_option( WpPerformance::OPTION_KEY . '_settings', array() );
		if ( isset( $settings['remove_querystrings'] ) && 1 === $settings['remove_querystrings'] ) {
	...
	public function wp_performace_disable_woo_stuffs() {
		$settings = get_option( WpPerformance::OPTION_KEY . '_settings', array() );
		if ( isset( $settings['disable_woocommerce_non_pages'] ) && 1 === $settings['disable_woocommerce_non_pages'] ) {
	...
	public function wp_performance_disable_google_maps() {
		$settings = get_option( WpPerformance::OPTION_KEY . '_settings', array() );
		if ( isset( $settings['disable_google_maps'] ) && 1 === $settings['disable_google_maps'] ) {
	...
	public function wp_performance_dequeue_woocommerce_cart_fragments() {
		$settings = get_option( WpPerformance::OPTION_KEY . '_settings', array() );
		if ( isset( $settings['disable_woocommerce_cart_fragments'] ) && 1 === $settings['disable_woocommerce_cart_fragments'] ) {
			if ( is_front_page() ) {
				wp_dequeue_script( 'wc-cart-fragments' );
			}
		}
